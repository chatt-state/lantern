/**
 * HTTP transport for the M365 MCP server.
...
import { createM365Server } from './server.js';
...
async function handleMcp(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const mcpServer = createM365Server();
...
const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  if (url === '/health' || url === '/health/') {
    ...
    res.end(JSON.stringify({ status: 'ok', server: 'lantern-m365' }));
    return;
  }
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`M365 MCP server listening on port ${PORT}`);
});
